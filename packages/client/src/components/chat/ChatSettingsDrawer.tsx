              ...
              <div className="space-y-2">
                ...
                <div ref={scheduleControlsRef} className="scroll-mt-2 space-y-2">
                  ...
                  {conversationSchedulesEnabled && hasGeneratedConversationSchedules && (
                    <div className="space-y-1.5 rounded-lg border border-[var(--border)] bg-[var(--secondary)]/20 p-2.5">
                      <div>
                        <span className="text-[0.625rem] font-medium text-[var(--muted-foreground)]">Edit schedules</span>
                        <p className="text-[0.55rem] text-[var(--muted-foreground)]/70">
                          Open one character at a time in a popup editor.
                        </p>
                      </div>
                      <div className="grid gap-1.5 sm:grid-cols-2">
                        {chatCharIds.map((charId) => {
                          const name = charNameMap.get(charId) ?? "Unknown";
                          const schedule = metadata.characterSchedules?.[charId];
                          return (
                            <button
                              key={charId}
                              type="button"
                            ...
                            >
                              <div className="min-w-0 flex-1">
                                ...
                                <span className="block text-[0.5625rem] text-[var(--muted-foreground)]">
                                  {schedule ? `${Object.keys(schedule.days).length} days scheduled` : "Create schedule"}
                                </span>
                              </div>
                              <ChevronRight size="0.6875rem" className="shrink-0 text-[var(--muted-foreground)]" />
                            </button>
                          );
                        })}
                      </div>
